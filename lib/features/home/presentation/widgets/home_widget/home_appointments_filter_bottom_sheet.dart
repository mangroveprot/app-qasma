...
enum AppointmentOverdueFilter { all, overdue, notOverdue }

enum AppointmentSortOrder { ascending, descending }

class HomeAppointmentsFilter {
  final DateTime? dateFrom;
  final DateTime? dateTo;
  final AppointmentOverdueFilter overdue;
  final AppointmentSortOrder sortOrder;

  const HomeAppointmentsFilter({
    this.dateFrom,
    this.dateTo,
    this.overdue = AppointmentOverdueFilter.all,
    this.sortOrder = AppointmentSortOrder.descending,
  });

  HomeAppointmentsFilter copyWith({
    DateTime? dateFrom,
    DateTime? dateTo,
    AppointmentOverdueFilter? overdue,
    AppointmentSortOrder? sortOrder,
  }) {
    return HomeAppointmentsFilter(
  ...
  HomeAppointmentsFilter reset() => const HomeAppointmentsFilter();

  HomeAppointmentsFilter resetDate() => copyWith(dateFrom: null, dateTo: null);

  HomeAppointmentsFilter resetOverdue() =>
      copyWith(overdue: AppointmentOverdueFilter.all);

  HomeAppointmentsFilter resetSort() =>
      copyWith(sortOrder: AppointmentSortOrder.descending);

  int get activeFilterCount {
    var count = 0;
    if (dateFrom != null || dateTo != null) count++;
    if (overdue != AppointmentOverdueFilter.all) count++;
    if (sortOrder != AppointmentSortOrder.descending) count++;
    return count;
  }
...
class HomeAppointmentsFilterBottomSheet extends StatefulWidget {
  final HomeAppointmentsFilter initialFilter;
  final Function(HomeAppointmentsFilter) onApply;

  const HomeAppointmentsFilterBottomSheet({
    super.key,
  ...
  @override
  State<HomeAppointmentsFilterBottomSheet> createState() =>
      _HomeAppointmentsFilterBottomSheetState();
}

class _HomeAppointmentsFilterBottomSheetState
    extends State<HomeAppointmentsFilterBottomSheet> {
  late HomeAppointmentsFilter _currentFilter;
                            ...
                            children: [
                              Expanded(
                                child: FilterDateField(
                                  label: 'From',
                                  date: _normalizeDate(_tempDateFrom),
                              ...
                              Expanded(
                                child: FilterDateField(
                                  label: 'To',
                                  date: _normalizeDate(_tempDateTo),
                                  onTap: _selectDateTo,
                                ),
                              ),
                            ],
                      ...
                      child: Wrap(
                        ...
                        children: [
                          FilterStatusChip(
                            value: 'all',
                            label: 'All',
                            isSelected: _currentFilter.overdue ==
                                AppointmentOverdueFilter.all,
                            dotColor: colors.textPrimary.withOpacity(0.5),
                            onTap: () => setState(
                              () => _currentFilter = _currentFilter.copyWith(
                                overdue: AppointmentOverdueFilter.all,
                              ),
                            ),
                          ),
                          FilterStatusChip(
                            ...
                            onTap: () => setState(
                              () => _currentFilter = _currentFilter.copyWith(
                                overdue: AppointmentOverdueFilter.overdue,
                              ),
                            ),
                          ),
                          FilterStatusChip(
                            ...
                            label: 'Not overdue',
                            isSelected: _currentFilter.overdue ==
                                AppointmentOverdueFilter.notOverdue,
                            dotColor: colors.secondary,
                            onTap: () => setState(
                              () => _currentFilter = _currentFilter.copyWith(
                                overdue: AppointmentOverdueFilter.notOverdue,
                              ),
                            ),
                          ),
                        ],
                    ...
                    FilterSection(
                      ...
                      child: _SortPicker(
                        ...
                        onChanged: (v) => setState(
                          () => _currentFilter =
                              _currentFilter.copyWith(sortOrder: v),
                        ),
                      ),
                    ),
                  ],
...
class _SortPicker extends StatelessWidget {
  final AppointmentSortOrder value;
  final ValueChanged<AppointmentSortOrder> onChanged;
  ...
  Widget build(BuildContext context) {
    final colors = context.colors;
    final label = value == AppointmentSortOrder.ascending
        ? 'Ascending'
        : 'Descending';
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          onChanged: (v) {
            if (v == 'Ascending') return onChanged(AppointmentSortOrder.ascending);
            return onChanged(AppointmentSortOrder.descending);
          },
        ),
      ),
    );
