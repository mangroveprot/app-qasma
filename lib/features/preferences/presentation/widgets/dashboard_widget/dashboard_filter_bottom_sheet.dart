...
import '_filter/filter_date_field.dart';
import '_filter/filter_quick_date_chip.dart';
import '_filter/filter_section.dart';
import '_filter/filter_status_chip.dart';

class DashboardFilterBottomSheet extends StatefulWidget {
  final AppointmentFilterModel initialFilter;
  final Function(AppointmentFilterModel) onApply;

  const DashboardFilterBottomSheet({
    super.key,
  ...
  @override
  State<DashboardFilterBottomSheet> createState() =>
      _DashboardFilterBottomSheetState();
}

class _DashboardFilterBottomSheetState
    extends State<DashboardFilterBottomSheet> {
  late AppointmentFilterModel _currentFilter;
  ...
  Color? _statusDotColor(String value) {
  ...
  Widget build(BuildContext context) {
    final colors = context.colors;
    final categories = _resolveCategories(context);
                ...
                child: Column(
                  children: [
                      ...
                      child: Wrap(
                        ...
                        children: [
                          ...
                          'cancelled',
                            ...
                            .map((value) => FilterStatusChip(
                                  value: value,
                                  label: value[0].toUpperCase() +
                                      value.substring(1),
                                  isSelected: _currentFilter.status == value,
                                  dotColor: _statusDotColor(value),
                                  onTap: () => setState(() => _currentFilter =
                                      _currentFilter.copyWith(status: value)),
                                ))
                            .toList(),
                      ),
                    ),
                    ...
                    FilterSection(
                      title: 'Purpose',
                      onReset: () => setState(
                          () => _currentFilter = _currentFilter.resetPurpose()),
                      child: _buildDropdown(
                        value: _currentFilter.purpose,
                        items: [
                          const DropdownMenuItem(
                              value: 'all', child: Text('All Purposes')),
                          ...categories.map((c) => DropdownMenuItem(
                                value: c.toLowerCase(),
                                child: Text(_formatCategoryLabel(c)),
                              )),
                        ],
                        onChanged: (v) => setState(() => _currentFilter =
                            _currentFilter.copyWith(purpose: v ?? 'all')),
                      ),
                    ),
  ...
  List<String> _resolveCategories(BuildContext context) {
    try {
      final configCubit = context.read<AppointmentConfigCubit>();
      final fromConfig = configCubit.allCategories;
      if (fromConfig.isNotEmpty) return fromConfig;
    } catch (_) {}
    return newAppointmentOptions.map((o) => o.value).toList();
  }
}
